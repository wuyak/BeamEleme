...
%
% 示例:
%   ana_result = analytical.axial.solve(params, 'CF', 10);
%   x = linspace(0, params.L, 100);
%   u_modes = ana_result.getModes(x);  % 100×10矩阵
...
% 计算频率
freq_Hz = analytical.axial.getFrequencies(params, BC, num_modes);
omega = freq_Hz * 2 * pi;
...
% 创建模态获取函数
L = params.L;
getModes = @(x) analytical.axial.getModes(x, L, BC, num_modes);
...
% 添加可视化所需的标准字段
ana_result.matrix_method = 'axial';
